...
extern void blockLU(double *L, double *U, int mm, int rank);
extern void	LUSolver(double *L, double *U, double *f, int N,int n, int rank, int size);
...
void mpi_dgemv(const MKL_INT m, const MKL_INT local_m, const double *local_A,const double *u, double *v, double *local_u, double *local_v,int taskid, MPI_Comm comm){
	...
	MPI_Scatter(u,local_m, MPI_DOUBLE, local_u, local_m, MPI_DOUBLE, MASTER, MPI_Comm comm   ); // send u_i from node MASTER to all other nodes.
